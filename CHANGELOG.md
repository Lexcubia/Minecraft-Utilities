...
- **应用内更新**：检查更新与「测试版」渠道一致，可升级到 beta / 预发布版本（不再仅依赖 GitHub Latest 稳定版）；下载进度进入顶栏 **通知收件箱**；安装前 **确认对话框** 可展示对应版本的变更摘要（解析自本仓库 `CHANGELOG.md`）。
...
- **界面（Linear 风格）**：在保留主题色、渐变与毛玻璃的前提下，收紧排版与信息密度；侧栏激活项左侧主色指示、紧凑列表与分区标题；欢迎页与更新页发布列表更贴近 Linear 式列表展示。
- **壳层**：侧栏宽度 240px；顶栏/设置栏与访问标签带更紧凑；Vuetify 默认密度与圆角略收。

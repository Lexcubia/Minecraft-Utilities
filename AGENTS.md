...
- **CI**：GitHub Actions 见 [`.github/workflows/`](.github/workflows/)，命名与职责参考 [MaaEnd v2 workflows](https://github.com/MaaEnd/MaaEnd/tree/v2/.github/workflows)：**`check`**（格式 + 全量 lint + Ruff）、**`test`**（Vitest + Pytest）、**`commitlint`**（仅 PR）、**`release`**（仅在 GitHub **Release 正式发布**时触发：Ubuntu / Windows / macOS 矩阵 **`node ./node_modules/@tauri-apps/cli/tauri.js build`**，将 **deb / AppImage / MSI / NSIS / DMG** 等 bundle 上传到该 Release 资源；日常 push/PR 不构建桌面安装包）；`check` / `test` 支持 **`workflow_dispatch`** 手动重跑。

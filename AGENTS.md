...
- **Windows 免安装包**：`pnpm tauri:build:portable`（前端 + `tauri build --no-bundle` + zip）或完整构建后 `pnpm desktop:pack:portable`；产出 `artifacts/portable/minecraft-utilities-win-{x86_64|aarch64}-v<x.y.z>.zip`（逻辑见 `scripts/pack-windows-portable.mjs`）。Linux：`pnpm desktop:pack:linux`（`scripts/pack-linux-tarball.mjs`，需先在 Linux 上 `tauri build --no-bundle`）。CI 见 `desktop-release.yml`。

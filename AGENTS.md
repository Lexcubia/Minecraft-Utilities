...
- **CI**：GitHub Actions 见 [`.github/workflows/`](.github/workflows/)：**`check`**（Prettier + ESLint + Markdownlint + Ruff + **`cargo check`** 于 `src-tauri`）、**`test`**（Vitest + Pytest）、**`commitlint`**、**`changelog-publish-marker`** 均在 **PR → `main`/`master`** 上运行，作为合并前门禁；**`develop`/功能分支 push 不触发**上述检查。**`main` 合并后**仅由 **`desktop-release.yml`** 根据 CHANGELOG 发布标记打包发版（`tauri build`，含 Rust 编译）；**不在 `main` push 时重复跑 `check`/`test`**。**`cargo fmt`/`clippy` 不在 PR 门禁中**（仅 `pnpm check:tauri` / `cargo check`）。修改 `src-tauri` 时 trait 与 `#[cfg]` 须同模块一致（见 `src-tauri/src/lib.rs` 注释）。详见 [docs/zh-cn/REPO_SETUP.md](docs/zh-cn/REPO_SETUP.md)。各工作流支持 **`workflow_dispatch`**。

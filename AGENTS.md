...
- **构建产物目录**：统一在仓库根 **`build/`** 下（`build/web`、`build/cargo-target`、`build/desktop`；详见 **`scripts/build-artifacts.mjs`**、**`src-tauri/.cargo/config.toml`**）。**`tauri.conf.json`** 中 **`bundle.active: false`**，避免默认生成 NSIS/MSI/DMG 等安装包；`pnpm tauri:build` 等价 **`tauri build --ci --no-bundle`**。免安装：**`pnpm desktop:build:windows-portable`**（或 `tauri:build:portable`）/ `desktop:pack:portable`（Windows）、`desktop:pack:linux`、`desktop:pack:macos`（需先完成对应 `tauri build`）。CI 校验见 **`scripts/assert-release-assets.mjs`** 与 **`desktop-release.yml`**（六款 zip/tar.gz）。
